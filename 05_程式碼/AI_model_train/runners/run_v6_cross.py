# ===========================================================================
# [交接導向註解]
# runner：accuracy-best 跨資料集 M=4。
# 流程：AI_model_train。產出對應 results/ 之 results.csv（見 README 對照表）。
# ===========================================================================

"""
v6 design × cross-dataset M=4 ablation.

Question: does the SVHN SW-best recipe (v6 design) also beat v1/v2 design on the
3 new datasets (STL10/FashionMNIST/CINIC10)? v9 only tested v1/v2 design.

Cells: 3 datasets × M=4 × {norc, rc} = 6 cells, 1-bit, 200ep.

Adapter design (matches v7 v6-eq, mid_basis='in' to mirror v7's headline 83.31):
  kernel=3, act=relu (sign at 1-bit), alpha=per_channel, bias toggleable, mid='in'

Output:
  claude/paper_results_bitwidth/v_v6_cross/
    experiments/Transfer_v6cross_{dataset}_M4_{rc|norc}_e200/
    logs/{dataset}_M4_{rc|norc}.log
...
OUTPUT_ROOT = os.path.join(THIS_DIR, 'paper_results_bitwidth', 'v_v6_cross')
...
def build_cmd(dataset, rc_on, args):
    bp = os.path.join(BACKBONE_DIR, f'cifar10_{BIT}w{BIT}a.tar')
    rc_tag = 'rc' if rc_on else 'norc'
    exp_name = f"Transfer_v6cross_{dataset}_M{M}_{rc_tag}_e{args.epochs}"
    ...
    cmd = [
        ...
        '--adapter_kernel', '3',           # v6 design
        '--adapter_act', 'relu',
        '--adapter_alpha', 'per_channel',
        '--adapter_mid_basis', 'in',       # match v7 v6-eq best
        '--no_rc',
    ]
    if rc_on:
...
def main():
    ...
    print(f"v_v6_cross sweep: 3 datasets x M=4 x 2 RC = 6 cells, 1-bit, "
          f"epochs={args.epochs}, parallel={args.parallel}, gpu={args.gpu}")

    jobs = []
    for dataset in DATASETS:
        for rc_on in RC_VALUES:
            _label, _cmd = build_cmd(dataset, rc_on, args)
            if is_done(_label):
    ...
    df = pd.DataFrame(rows).sort_values(['dataset', 'rc']).reset_index(drop=True)
