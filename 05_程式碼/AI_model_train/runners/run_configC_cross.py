# ===========================================================================
# [交接導向註解]
# runner：accuracy-best 1-bit 跨資料集。
...
def build_full_ft(source, target, args, exp_dir):
    ...
    exp_name = f"Transfer_{source.lower()}2oCC_{target}_full_ft_e{args.epochs}"
...
def build_adapter(source, target, M, rc_on, args, exp_dir):
    ...
    exp_name = f"Transfer_{source.lower()}2oCC_{target}_M{M}_{rc_tag}_e{args.epochs}"
    ...
    cmd = [
        ...
        '--adapter_alpha', 'per_channel', '--adapter_mid_basis', 'out',
        '--no_rc',
    ]
    if rc_on:
...
def main():
    ...
    output_root = os.path.join(THIS_DIR, 'paper_results_bitwidth',
                               f'{args.source.lower()}_configC_cross')
