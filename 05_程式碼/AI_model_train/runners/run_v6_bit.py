# ===========================================================================
# [交接導向註解]
# runner：v6 × bit-width × M=4。
# 流程：AI_model_train。產出對應 results/ 之 results.csv（見 README 對照表）。
# ===========================================================================

"""
v6 design × bit-width × M=4 RC sweep.

Question: does v6+M=4+RC remain the best recipe across all bit-widths,
or does it only matter at 1-bit?

Cells: 6 bits {1,2,4,8,16,32} × M=4 × RC=on = 6 cells, SVHN, 200ep.

Adapter design (matches v7 v6-eq, mid_basis='in'):
  kernel=3, act=relu (sign at 1-bit), alpha=per_channel, bias=on, mid='in'

Output:
  claude/paper_results_bitwidth/v_v6_bit/
    experiments/Transfer_v6bit_b{N}_M4_rc_e200/
    logs/b{N}_M4_rc.log
    results.csv  bit,M,rc,acc,params,returncode
"""
...
OUTPUT_ROOT = os.path.join(THIS_DIR, 'paper_results_bitwidth', 'v_v6_bit')
...
def build_cmd(bit, args):
    ...
    exp_name = f"Transfer_v6bit_b{bit}_M{M}_{rc_tag}_e{args.epochs}"
    label = f"b{bit}_M{M}_{rc_tag}"
    ...
    cmd = [
        ...
        '--num_branches', str(M),
        '--adapter_bit_width', str(bit),  # matched-bit adapter (per v2 design choice)
...
def main():
    ...
    print(f"v_v6_bit sweep: {len(BITS)} bits x M=4 x RC=on = {len(BITS)} cells, "
          f"v6 design, SVHN, epochs={args.epochs}, parallel={args.parallel}, gpu={args.gpu}")

    jobs = []
    for bit in BITS:
        _label, _cmd = build_cmd(bit, args)
        if is_done(_label):
    ...
    for label, acc, params, rc in results:
        ...
        parts = label.split('_')
        bit = int(parts[0][1:])
        rows.append({'bit': bit, 'M': M, 'rc': RC_ON, 'acc': acc,
