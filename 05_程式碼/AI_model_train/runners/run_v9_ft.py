...
Mirrors v9's hyperparams but with `--mode full_ft`: backbone unfrozen, no adapter.
Gives the per-dataset ceiling so we can quote "adapter vs full_ft gap" across
STL10 / FashionMNIST / CINIC10 the way SVHN already has (94.91%).

3 datasets x 1 cell = 3 cells, 1-bit, 200ep.

Output:
  claude/paper_results_bitwidth/v9_ft_baseline/
    experiments/Transfer_v9ft_{dataset}_full_ft_e200/
    logs/{dataset}_full_ft.log
    results.csv  dataset,acc,params,returncode
"""
...
OUTPUT_ROOT = os.path.join(THIS_DIR, 'paper_results_bitwidth', 'v9_ft_baseline')
...
def build_cmd(dataset, args):
    ...
    exp_name = f"Transfer_v9ft_{dataset}_full_ft_e{args.epochs}"
    label = f"{dataset}_full_ft"
...
def main():
    ...
    print(f"v9_ft sweep: {len(args.datasets)} datasets full_ft, 1-bit, "
          f"epochs={args.epochs}, parallel={args.parallel}, gpu={args.gpu}")

    jobs = [build_cmd(d, args) for d in args.datasets]
    ...
    for label, acc, params, rc in results:
        dataset = label.replace('_full_ft', '')
        rows.append({'dataset': dataset, 'acc': acc, 'params': params, 'returncode': rc})
