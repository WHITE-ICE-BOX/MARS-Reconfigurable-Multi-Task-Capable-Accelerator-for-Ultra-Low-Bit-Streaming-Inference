# ===========================================================================
# [交接導向註解]
# runner：跨來源 bit-width sweep（1/2/4/8-bit）。
...
Cells: |targets|=4 x |bits| x 2 modes (adapter_norc, full_ft).
...
def build_full_ft(source, target, bit, args, exp_dir):
    ...
    exp_name = f"Transfer_{source.lower()}2obits_{target}_b{bit}_full_ft_e{args.epochs}"
...
def build_adapter_norc(source, target, bit, args, exp_dir):
    bp = os.path.join(BACKBONE_DIR, f'{source.lower()}_{bit}w{bit}a.tar')
    exp_name = f"Transfer_{source.lower()}2obits_{target}_b{bit}_adapter_norc_e{args.epochs}"
    label = f"{target}_b{bit}_adapter_norc"
    ms = f"{int(args.epochs * 0.5)},{int(args.epochs * 0.75)}"
    cmd = [
        sys.executable, '-u', TRAIN_SCRIPT,
...
def parse_label(label):
    """label = '{dataset}_b{N}_{full_ft|adapter_norc}'."""
    parts = label.split('_')
    dataset = parts[0]
    bit = int(parts[1][1:])
    mode = '_'.join(parts[2:])
...
def main():
    ...
    p.add_argument('--modes', type=str, nargs='+',
                   default=['adapter_norc', 'full_ft'],
                   choices=['adapter_norc', 'full_ft'])
    ...
    output_root = os.path.join(THIS_DIR, 'paper_results_bitwidth',
                               f'{args.source.lower()}_to_others_bits')
    exp_dir = os.path.join(output_root, 'experiments')
    log_dir = os.path.join(output_root, 'logs')
    os.makedirs(exp_dir, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)

    targets = [d for d in CANONICAL if d != args.source]
        ...
        for bit in args.bits:
            if 'full_ft' in args.modes:
                ...
                if is_done(log_dir, _l):
                    ...
                    jobs.append((_l, _c))
            if 'adapter_norc' in args.modes:
                _l, _c = build_adapter_norc(args.source, tgt, bit, args, exp_dir)
                if is_done(log_dir, _l):
                    print(f"SKIP done: {_l}")
                else:
                    jobs.append((_l, _c))
