...
def build_full_ft(source, target, args, exp_dir):
    ...
    exp_name = f"Transfer_{source.lower()}2o_{target}_full_ft_e{args.epochs}"
...
def build_adapter(source, target, M, rc_on, args, exp_dir):
    ...
    exp_name = f"Transfer_{source.lower()}2o_{target}_M{M}_{rc_tag}_e{args.epochs}"
    ...
    cmd = [
        ...
        '--adapter_kernel', '3', '--adapter_act', 'relu',
        '--adapter_alpha', 'per_channel', '--adapter_mid_basis', 'in',
        '--no_rc',
    ]
    if rc_on:
...
def main():
    ...
    output_root = os.path.join(THIS_DIR, 'paper_results_bitwidth',
                               f'{args.source.lower()}_to_others')
    ...
    jobs = []
    for tgt in targets:
        _l, _c = build_full_ft(args.source, tgt, args, exp_dir)
