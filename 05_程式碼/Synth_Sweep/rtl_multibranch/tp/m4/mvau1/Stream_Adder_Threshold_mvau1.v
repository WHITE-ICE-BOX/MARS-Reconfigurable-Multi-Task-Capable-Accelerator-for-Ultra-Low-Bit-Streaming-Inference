...
// Stream_Adder_Threshold_mvau1 — multi-branch M=4, PE=32 (throughput),
...
module Stream_Adder_Threshold_mvau1 (
    ...
    parameter PE        = 32;
    parameter OUT_CH    = 64;
    parameter OUT_STEPS = 2;

    localparam CFG_THRESH_BASE = 11'd1152;
    localparam CFG_SIGN_BASE   = 11'd1408;
    localparam signed [31:0] CONTRIB_OFFSET = 24;
    localparam signed [31:0] ALPHA_Q8_B0 = 911;  // baked from checkpoint
    localparam signed [31:0] ALPHA_Q8_B1 = 912;  // baked from checkpoint
    localparam signed [31:0] ALPHA_Q8_B2 = 839;  // baked from checkpoint
    localparam signed [31:0] ALPHA_Q8_B3 = 813;  // baked from checkpoint
    ...
    generate
        for (gp = 0; gp < PE; gp = gp + 1) begin : g_lane
            ...
            always @(posedge aclk) begin
                if (cfg_wen) begin
                    if (cfg_waddr >= CFG_THRESH_BASE && cfg_waddr < CFG_THRESH_BASE + OUT_CH
                        ...
                        th_mem[(cfg_waddr - CFG_THRESH_BASE) >> 5] <= $signed(cfg_wdata);
                    if (cfg_waddr >= CFG_SIGN_BASE && cfg_waddr < CFG_SIGN_BASE + OUT_CH
                        && ((cfg_waddr - CFG_SIGN_BASE) & (PE-1)) == gp)
                        sg_mem[(cfg_waddr - CFG_SIGN_BASE) >> 5] <= cfg_wdata[0];
                end
            end
            ...
            reg signed [31:0] s1_contrib_b3;
            reg signed [31:0] s1_threshold;
            reg               s1_adp_sign;
            reg signed [31:0] s2_total_q8;
            reg signed [31:0] s2_threshold;
            wire signed [31:0] contrib_sum = s1_contrib_b0 + s1_contrib_b1 + s1_contrib_b2 + s1_contrib_b3;
            ...
            always @(posedge aclk) begin
                if (!aresetn) begin
                    ...
                    if (in_fire) begin
                        ...
                        s1_contrib_b3 <= ($signed({24'd0, s_axis_2_b3_tdata[(gp*8) +: 8]}) - CONTRIB_OFFSET) * ALPHA_Q8_B3;
