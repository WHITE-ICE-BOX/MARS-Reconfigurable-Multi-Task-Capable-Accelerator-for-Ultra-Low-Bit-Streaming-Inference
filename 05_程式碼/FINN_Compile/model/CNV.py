...
class MultiBranchAdapter(Module):
    """
    Single-branch adapter (state_dict format: adapters.X.{down,up,alpha}).
    Matches v7_multi_rc/v9_cross_dataset trained checkpoints for M=1.
    """
    def __init__(self, in_channels, out_channels, num_branches=1, bit_width=1, rc_bit_width=8, use_rc=True):
        ...
        assert num_branches == 1, f"Only M=1 supported; got num_branches={num_branches}"
        self.use_rc = use_rc
        reduced_channels = max(1, in_channels // 4)
        bias_quantizer = Int8Bias if rc_bit_width == 8 else None

        self.down = QuantConv2d(
            in_channels=in_channels, out_channels=reduced_channels,
            kernel_size=1, bias=True, bias_quant=bias_quantizer,
            weight_quant=CommonWeightQuant, weight_bit_width=bit_width)
        self.act = QuantIdentity(act_quant=CommonActQuant, bit_width=bit_width)
        self.up = QuantConv2d(
            in_channels=reduced_channels, out_channels=out_channels,
    ...
    def forward(self, x):
        ...
        alpha_val = self.alpha if self.training else float(self.alpha)
        return out * alpha_val
