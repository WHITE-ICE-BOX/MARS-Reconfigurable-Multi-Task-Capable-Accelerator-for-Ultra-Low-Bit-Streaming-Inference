...
// Stream_Adder_Threshold_mvau4 — multi-branch M=4, PE=4 (throughput),
...
module Stream_Adder_Threshold_mvau4 (
    ...
    parameter OUT_STEPS = 64;

    localparam CFG_THRESH_BASE = 11'd1152;
    localparam CFG_SIGN_BASE   = 11'd1408;
    localparam signed [31:0] CONTRIB_OFFSET = 16;
    localparam signed [31:0] ALPHA_Q8_B0 = 463;  // baked from checkpoint
    localparam signed [31:0] ALPHA_Q8_B1 = 465;  // baked from checkpoint
    localparam signed [31:0] ALPHA_Q8_B2 = 493;  // baked from checkpoint
    localparam signed [31:0] ALPHA_Q8_B3 = 504;  // baked from checkpoint
            ...
            always @(posedge aclk) begin
                if (cfg_wen) begin
                    if (cfg_waddr >= CFG_THRESH_BASE && cfg_waddr < CFG_THRESH_BASE + OUT_CH
                        ...
                        th_mem[(cfg_waddr - CFG_THRESH_BASE) >> 2] <= $signed(cfg_wdata);
                    if (cfg_waddr >= CFG_SIGN_BASE && cfg_waddr < CFG_SIGN_BASE + OUT_CH
                        && ((cfg_waddr - CFG_SIGN_BASE) & (PE-1)) == gp)
                        sg_mem[(cfg_waddr - CFG_SIGN_BASE) >> 2] <= cfg_wdata[0];
                end
            end
