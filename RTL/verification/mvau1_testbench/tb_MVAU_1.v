...
module tb_MVAU_1;
    ...
    // ✨ MVAU1 規格 (1024-bit weights/MAC, 256-bit Popcount)
    ...
    wire [255:0]  adapt_to_fifo_tdata;
    wire          adapt_to_fifo_tvalid;
    wire          adapt_to_fifo_tready;

    wire [255:0]  fifo_to_add_tdata;
    ...
    initial begin
        ...
        $display("---- System Reset Done. Starting Adapter Subsystem Data Feed (MVAU1) ----");
        tb_in_tvalid = 1;
        ...
        $display("----------------------------------------");
        if (err_cnt == 0) $display("🎉🎉🎉 驗證通過！MVAU1 Adapter 融合後硬體與軟體 100%% 吻合！");
        else begin
    ...
    always @(posedge ap_clk) begin
        if (tb_out_tvalid && tb_out_tready) begin
            if (tb_out_tdata[31:0] !== golden_out[out_ptr][31:0]) begin
                $display("--------------------------------------------------");
                $display("❌ [錯誤] 序號: %0d (Channel 群: %0d ~ %0d)", out_ptr, step*32, step*32+31);
                $display("   -> 硬體輸出: %032b | 預期輸出: %032b", tb_out_tdata[31:0], golden_out[out_ptr][31:0]);
                $display("--------------------------------------------------");
                err_cnt = err_cnt + 1;
            end
            else begin
                cor_cnt = cor_cnt + 1;
                if (cor_cnt % 2000 == 0) $display("🟢 [進度回報] 目前已成功比對 %0d 筆資料...", cor_cnt);
            end
            out_ptr = out_ptr + 1;
        end
    end
