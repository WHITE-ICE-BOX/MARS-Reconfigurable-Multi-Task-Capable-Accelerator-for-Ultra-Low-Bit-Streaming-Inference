...
// =====================================================================
// MVAU1 Stream_Adder_Threshold — PE=1 version
// =====================================================================
// Per beat: 1 OC. ch_cnt sweeps 0..OUT_CH-1.
//   contrib   = adp_contrib_lut[adp_pop]
//   signed_c  = adp_sign_rom[ch] ? -contrib : contrib
//   total_q8  = (mvau_pop << 8) + signed_c * adapter_enable
//   out_bit   = (total_q8 >= thresh_rom[ch])
// All ROMs cfg-writable RAM (no $readmemh).
...
`ifdef SIM_INIT_ROM
    initial begin
        $readmemh("/home/barkie1/mvau_pipeline_runtime_3ds_pe1/mvau_adapter/mvau1/data/thresh_load.dat",      thresh_rom);
        $readmemh("/home/barkie1/mvau_pipeline_runtime_3ds_pe1/mvau_adapter/mvau1/data/sign_load.dat",        adp_sign_rom);
        $readmemh("/home/barkie1/mvau_pipeline_runtime_3ds_pe1/mvau_adapter/mvau1/data/contrib_lut_load.dat", adp_contrib_lut);
    end
`endif
    ...
    // OC counter: sequential 0..OUT_CH-1, wraps per pixel
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            // Stage 2 (widen + add adapter contribution)
            if (s1_valid) begin
                s2_valid <= 1'b1;
                if (adapter_enable)
