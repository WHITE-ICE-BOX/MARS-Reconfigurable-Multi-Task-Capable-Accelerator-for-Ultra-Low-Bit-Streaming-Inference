...
try:
    ...
    try:
        ...
        print("❌ 警告: 找不到 common 或 tensor_norm 模組！請確認你是在專案根目錄執行此腳本。")
        exit(1)
...
class PowerMonitor:
    def __init__(self):
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                self.mode = "Jetson (tegrastats)"
        except Exception:
            self.mode = "Unknown"

    def _read_power_nvml(self):
        import pynvml
        try:
    ...
    def start(self):
        ...
        if self.mode == "Jetson (tegrastats)":
    ...
    def _monitor_loop(self):
        if self.mode == "Server (NVML)":
            while self.keep_running:
                p = self._read_power_nvml()
                if p > 0:
                    self.power_readings.append(p)
                time.sleep(0.1)

        elif self.mode == "Jetson (tegrastats)":
            power_pattern = re.compile(r'([A-Z0-9_]+)\s+(\d+)mW/\d+mW')

            while self.keep_running and self.proc and self.proc.stdout:
                line = self.proc.stdout.readline()
                if not line:
                    continue

                rails = {name: int(mw) for name, mw in power_pattern.findall(line)}

                if "VDD_IN" in rails:
                    total_w = rails["VDD_IN"] / 1000.0
                elif "VIN_SYS_5V0" in rails:
                    total_w = rails["VIN_SYS_5V0"] / 1000.0
                elif "VDD_CPU_GPU_CV" in rails and "VDD_SOC" in rails:
                    total_w = (rails["VDD_CPU_GPU_CV"] + rails["VDD_SOC"]) / 1000.0
                elif "VDD_CPU_GPU_CV" in rails:
                    total_w = rails["VDD_CPU_GPU_CV"] / 1000.0
                else:
                    total_w = 0.0

                if total_w > 0:
                    self.power_readings.append(total_w)
...
def build_svhn_loader(batch_size=100, num_workers=0):
...
def evaluate_accuracy(model, testloader, total_images_limit=26000):
    print(f"\n📦 正在用 SVHN 測試準確率 (目標: {total_images_limit} 筆)...")
    ...
    print(f"✅ 完成 {total} 筆 SVHN 推論，準確率: {accuracy:.2f}%")
    return accuracy, total


def benchmark_gpu_fair(model, testloader, total_images_limit=26000, power_monitor=None, ops_per_img=0):
    print("\n" + "=" * 40)
    ...
    warmup_batch = testloader.batch_size
    print("🔥 開始 GPU 預熱...")
    warmup = torch.randn(warmup_batch, 3, 32, 32, device='cuda')
    with torch.no_grad():
        for _ in range(50):
            _ = model(warmup)
    ...
    latency_wall_img_ms = (total_wall_time / total_seen) * 1000.0
    latency_device_img_ms = (total_device_time / total_seen) * 1000.0
    latency_compute_img_ms = (total_compute / total_seen) * 1000.0

    efficiency_wall = fps_wall / avg_power if avg_power > 0 else 0.0
    ...
    print("=" * 40)
    print(f"🎯 模型準確率          : {acc:.2f} % (以 {total_seen} 筆 SVHN 樣本計算)")
    ...
    print(f"⏱️ End-to-End 每張延遲 : {latency_wall_img_ms:.4f} ms")
    print(f"⏱️ Device 每張延遲     : {latency_device_img_ms:.4f} ms")
    print(f"⏱️ Compute 每張延遲    : {latency_compute_img_ms:.4f} ms")
    ...
    if power_monitor is not None and power_monitor.mode != "Unknown":
        ...
        print(f"🏆 End-to-End 能效     : {efficiency_wall:.2f} img/s/W")
...
def main():
    ...
    _, testloader = build_svhn_loader(batch_size=BATCH_SIZE, num_workers=0)

    print("\n" + "=" * 40)
    print("階段一：SVHN 準確率測試")
    print("=" * 40)
    _, total_images = evaluate_accuracy(model, testloader, total_images_limit=TARGET_IMAGES)
    ...
    benchmark_gpu_fair(
        ...
        ops_per_img=ops_per_img  # 傳遞算好的運算量給 benchmark
    )
